...
class HearingPrepStatusEnumConverter : Converter<String, HearingPrepStatus> {
    override fun convert(source: String): HearingPrepStatus? {
        try { return HearingPrepStatus.valueOf(source) } catch (e: IllegalArgumentException) { throw HttpClientErrorException(HttpStatus.BAD_REQUEST, "Invalid value $source for hearing prep status") }
    }
}