...
public class ConvictionService {
    ...
    public CommunityApiSentenceStatusResponse getSentenceStatus(final String crn, final Long convictionId) {
        return convictionRestClient.getSentenceStatus(crn, convictionId)
                .blockOptional()
                .orElseThrow(() -> new CustodialStatusNotFoundException(crn));
    }

    private SentenceResponse combineOffenderAndConvictions(List<AttendanceResponse> attendanceResponses, Conviction conviction, CommunityApiSentenceStatusResponse sentenceStatus, CommunityApiOffenderResponse offenderDetail) {        var builder = SentenceResponse.builder()
