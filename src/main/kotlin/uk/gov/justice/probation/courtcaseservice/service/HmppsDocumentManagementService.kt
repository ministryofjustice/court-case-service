...
@Service
class HmppsDocumentManagementService(
  val hmppsDocumentManagementApiClient: HmppsDocumentManagementApiClient,
  ...
  fun deleteDocument(hearingId: String, defendantId: String, documentId: String): Optional<Mono<ResponseEntity<Any>>> = getHearingEntity(hearingId, defendantId)?.courtCase?.getCaseDefendant(defendantId)?.map { it.getCaseDefendantDocument(documentId) }
    ?.map {
