...
class CaseWorkflowController(val caseWorkflowService: CaseWorkflowService, val authenticationHelper: AuthenticationHelper) {
    ...
    fun updateHearingOutcome(@PathVariable("hearingId") hearingId: String, @RequestBody hearingOutcome: HearingOutcomeAssignRequest, principal: Principal) =
            caseWorkflowService.updateHearingOutcome(hearingId, HearingOutcomeItemState.IN_PROGRESS.name, hearingOutcome.assignedTo, authenticationHelper.getAuthUserUuid(principal))
