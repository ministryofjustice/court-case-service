...
public interface HearingRepository extends CrudRepository<HearingEntity, Long>, HearingSearchRepositoryCustom {
    ...
    @Query(value = "select distinct court_room from hearing_day " +
        "where court_code = :courtCode order by court_room",
        nativeQuery = true)
    List<String> getCourtroomsForCourt(String courtCode);

    @Modifying
    @Query(value = "INSERT INTO hearing_outcome " +
        " (outcome_type, created, created_by, version, outcome_date, state, fk_hearing_defendant_id) " +
    ...
    @Query(value = "INSERT INTO hearing_outcome " +
        ...
        " join hearing_defendant hd on hd.fk_hearing_id = h.id " +
        " left join hearing_outcome ho on ho.fk_hearing_defendant_id = hd.id   " +
        " where ho.fk_hearing_defendant_id_ is null",
        nativeQuery = true)
